import sys
sys.path.append('/home/cnzhang/kaggle/dsb2018/build/mask-rcnn-resnet50-ver-01/')
...
class BottleneckBlock(nn.Module):
    def __init__(self, in_planes, planes, out_planes, is_downsample=False, stride=1):
        ...
        self.bn3   = nn.BatchNorm2d(out_planes,eps = 2e-5)

        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != out_planes:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, out_planes, kernel_size=1, padding=0, stride=stride, bias=False),
                nn.BatchNorm2d(out_planes,eps = 2e-5)
            )
...
class FeatureNet(nn.Module):
    ...
    def __init__(self, cfg, in_channels, out_channels=256 ):
        ...
        self.layer_c1 = make_layer_c(   64,  64,  256, num_blocks=6, stride=1)  #out =  64*4 =  256
        self.layer_c2 = make_layer_c(  256, 128,  512, num_blocks=4, stride=2)  #out = 128*4 =  512
        self.layer_c3 = make_layer_c(  512, 256, 1024, num_blocks=3, stride=2)  #out = 256*4 = 1024
