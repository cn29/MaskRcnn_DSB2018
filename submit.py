...
def run_submit():
    ...
    out_dir  = RESULTS_DIR + '/mask-rcnn-all_670'
    initial_checkpoint = \
        out_dir + '/checkpoint/00013000_model.pth'
    ...
    for i, (inputs, images, indices) in enumerate(test_loader, 0):
        ...
        #print(images[0])
        #print(inputs)
        tmp1 = torch.FloatTensor()
        net.set_mode('test')
        with torch.no_grad():
            inputs = Variable(inputs).cuda()
            net(inputs )
            if net.detections.size() == tmp1.size():
                print('skip', i)
                continue
...
def run_npy_to_sumbit_csv():
    ...
    image_dir   = '/home/cnzhang/kaggle/dsb2018/data/image/stage1_test/images'

    submit_dir  = \
        '/home/cnzhang/kaggle/dsb2018/results/mask-rcnn-50-gray500-02/submit'
